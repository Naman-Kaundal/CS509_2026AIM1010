...
int main(int argc, char *argv[]) {
    ...
    // --- Timed region: adjacency-list -> CSR conversion only ---
    auto start = std::chrono::high_resolution_clock::now();
    CSRGraph csr = convertAdjListToCSR(adj, V, weighted);
    if (hasSource) {
        csr.source = source;
    }
    auto end = std::chrono::high_resolution_clock::now();
    ...
    std::cout << "Input file: " << inputPath << "\n";
    std::cout << "Graph type: " << (weighted ? "Weighted" : "Unweighted") << " adjacency list\n";
    std::cout << "V = " << csr.V << ", E (input) = " << E
              << ", CSR entries = " << csr.E << "\n";
    if (hasSource) std::cout << "Source: " << source << "\n";

    std::cout << "\nAlgorithm: Adjacency-List to CSR Conversion\n";

    auto printVecInt = [](const std::string &label, const std::vector<int> &v) {
        std::cout << label << " (" << v.size() << "): ";
        size_t limit = std::min<size_t>(v.size(), 50);
        for (size_t i = 0; i < limit; ++i) std::cout << v[i] << (i + 1 < limit ? ' ' : '\n');
        if (v.size() > limit) std::cout << "... (" << v.size() - limit << " more)\n";
        if (v.empty()) std::cout << "\n";
    };

    auto printVecDouble = [](const std::string &label, const std::vector<double> &v) {
        std::cout << label << " (" << v.size() << "): ";
        size_t limit = std::min<size_t>(v.size(), 50);
        for (size_t i = 0; i < limit; ++i) std::cout << v[i] << (i + 1 < limit ? ' ' : '\n');
        if (v.size() > limit) std::cout << "... (" << v.size() - limit << " more)\n";
    ...
    printVecInt("row_ptr", csr.row_ptr);
    printVecInt("col_idx", csr.col_idx);
    if (weighted) printVecDouble("values ", csr.values);
